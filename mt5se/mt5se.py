...
def get_shares(symbolId):
    ...
    FIXED: Now sums ALL positions to get the net position (not just first one)
"""
    global inbacktest
    global bts
    if inbacktest:
    ...
    if pos is not None and pos!=():
        # FIX: Sum ALL positions to get net position (not just first one)
        net_volume = 0.0
        for position in pos:
            if position.type == 1:  # SELL position
                net_volume -= position.volume
            else:  # BUY position (type == 0)
                net_volume += position.volume
        return net_volume
    else:
        return 0
